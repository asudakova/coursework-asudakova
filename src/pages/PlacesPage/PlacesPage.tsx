...
const Places: React.FC = () => {
    ...
    useEffect(() => {
        if (pageNumber > 1 && amountOfPages >= pageNumber) {
            dispatch(fetchNextPage({north, east, south, west, category, sortBy, pageNumber}))
        }
    }, [pageNumber]);
    ...
    const handleScroll = () => {
        if (blockRef.current?.scrollHeight!== undefined && blockRef.current?.clientHeight !== undefined && blockRef.current?.scrollTop !== undefined) {
            if (blockRef.current?.scrollHeight - (blockRef.current?.clientHeight + blockRef.current?.scrollTop) < 150) {
