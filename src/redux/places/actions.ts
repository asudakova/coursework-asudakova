...
export const fetchPlaces = ({ north, east, south, west, category, sortBy }: FetchArgumentsType) => async (dispatch: AppDispatch) => {
    ...
    if (cache.get(fetchString)) {
        ...
        try {
            ...
            if (response.data.meta.code == 404 && response.data.meta.error?.message == 'Results not found') {
                ...
                cache.set(fetchString, { mapPlaces, listIdPlaces, markerPlaces, total });
...
export const fetchNextPage = ({ north, east, south, west, category, sortBy, pageNumber }: FetchNextPageArgumentsType) => async (dispatch: AppDispatch) => {
    ...
    if (cache.get(fetchString)) {
        ...
        try {
            ...
            if (response.data.meta.code == 404 && response.data.meta.error?.message == 'Results not found') {
                ...
                cache.set(fetchString, { mapPlaces, listIdPlaces, markerPlaces });
